...
public class XYLineChart_AWT extends ApplicationFrame {
	...
	public XYLineChart_AWT(String applicationTitle, String chartTitle, String Xaxis, String Yaxis,
			ArrayList<Exp> constraints, int winX, int winY) {
		...
		// populationSeries is a collection of points holding the entire
		// population of points where each point is an individual organism
		...
		// TimeSeriesCollection(this.series);
	...
	private JFreeChart createChart(XYDataset dataset, String chartTitle, String Xaxis, String Yaxis) {
		JFreeChart xylineChart = ChartFactory.createXYLineChart(chartTitle, Xaxis, Yaxis,
				// samplexydataset2(),
	...
	public void dispConstraints() {
		...
		for (int index = 0; index < constraints.size(); index++) {
			 ...
			 * engine = mgr.getEngineByName("JavaScript"); String foo = "40+2";
			 * try { System.out.println(engine.eval(foo)); } catch
			 * (ScriptException e) { // TODO Auto-generated catch block
			 * e.printStackTrace(); }
			 */
		...
		// renderer.setSeriesStroke( xydataset.indexOf(fun2) , new BasicStroke(
		// 1.5f ) );
		// renderer.setSeriesShape( xydataset.indexOf(fun2),
	 ...
	 * String Xaxis, String Yaxis, int WindowLen, int WindowHght ) {
	 * super(applicationTitle); createDataset(); JFreeChart xylineChart =
	 * ChartFactory.createXYLineChart( chartTitle , Xaxis , Yaxis ,
	 * //samplexydataset2(), xydataset , PlotOrientation.VERTICAL , true , true
	 * , false);
	 * 
	 * ChartPanel chartPanel = new ChartPanel( xylineChart );
	 * chartPanel.setPreferredSize( new java.awt.Dimension( WindowLen ,
	 * WindowHght ) ); final XYPlot plot = xylineChart.getXYPlot( );
	 * 
	 * 
	 * 
	 * XYLineAndShapeRenderer renderer = new XYLineAndShapeRenderer( );
	 * renderer.setSeriesPaint( 0 , Color.BLUE ); renderer.setSeriesPaint( 1 ,
	 * Color.BLUE ); renderer.setSeriesPaint( 2 , Color.BLACK );
	 * renderer.setSeriesPaint( 3 , Color.BLACK ); renderer.setSeriesPaint( 4 ,
	 * Color.RED ); renderer.setSeriesPaint( 5 , Color.RED );
	 * renderer.setSeriesStroke( 0 , new BasicStroke( 1.5f ) );
	 * renderer.setSeriesStroke( 1 , new BasicStroke( 1.5f ) );
	 ...
	 * renderer.setSeriesLinesVisible(4,false);renderer.setSeriesLinesVisible(5,
	 * false);
	 * 
	 * plot.setRenderer( renderer );
	 * 
	 * setContentPane( chartPanel ); createDataset(); setContentPane( chartPanel
	 * );
	 * 
	 * }
	 */
	...
	/*
	 * private static XYDataset samplexydataset2() { // int cols = 20; // int
	 * rows = 20; // double[][] values = new double[cols][rows];
	 * 
	 * XYSeriesCollection xySeriesCollection = new XYSeriesCollection();
	 * XYSeries series = new XYSeries("Random"); // Random rand = new Random();
	 * for (int i = 0; i < values.length; i++) { for (int j = 0; j <
	 * values[i].length; j++) { double x = Math.round(rand.nextDouble() * 500);
	 * double y = Math.round(rand.nextDouble() * 500);
	 * 
	 * series.add(3,6);series.add(3,7);series.add(4,6); // } // }
	 * xySeriesCollection.addSeries(series); return
		...
		if (populationInitFlag) {
			...
			for (int i = 0; i < values.length; i++) {
				try {
					...
					System.out.println("PointUsageException: Point array index:" + i);
					e.printStackTrace();
				}
			}
		}
